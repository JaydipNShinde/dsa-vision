...
import { Play, RotateCcw, Shuffle, Info, ChevronDown, ChevronUp } from "lucide-react";
...
export default function SortingVisualizer() {
  ...
  const delay = useCallback(() => new Promise<void>((r) => setTimeout(r, Math.max(30, 1200 - speed * 11))), [speed]);
    ...
    <div className="max-w-6xl mx-auto space-y-5">
      ...
      <div className="glass rounded-xl p-4 flex flex-wrap items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-muted-foreground">{speed <= 15 ? "Very Slow" : speed <= 30 ? "Slow" : speed <= 50 ? "Medium" : speed <= 80 ? "Fast" : "Ultra"}</span>
        </div>
      ...
      {/* Visualization Area — Single unified bar+label view */}
      <div className="glass rounded-xl overflow-hidden p-6">
        <div className="flex items-end justify-center gap-[3px] h-80">
          <AnimatePresence>
            {array.map((val, i) => {
              ...
              return (
                <motion.div
                  ...
                  style={{ width: `${Math.max(2, 92 / array.length)}%`, height: "100%" }}
                >
                  {/* Value label on top */}
                  {array.length <= 35 && (
                    <motion.span
                      ...
                      style={{ color }}
                      animate={{ scale: isComp ? 1.3 : 1, y: isComp ? -4 : 0 }}
                  ...
                  <motion.div
                    ...
                    animate={{
                      height: `${heightPct}%`,
                      scaleX: isComp ? 1.15 : 1,
                      y: isComp ? -6 : 0,
                    }}
                    transition={{
                      height: { type: "spring", stiffness: 300, damping: 30 },
                      scaleX: { type: "spring", stiffness: 500, damping: 25 },
                      y: { type: "spring", stiffness: 500, damping: 25 },
                    }}
                    style={{
                      background: `linear-gradient(to top, ${color}, ${color}cc)`,
                      boxShadow: isComp || isSrt
                        ? `0 0 16px ${glowColor}, 0 -4px 20px ${glowColor}`
                        : "none",
                    }}
                  >
                    {/* Shimmer effect on sorted */}
                    {isSrt && (
                      <motion.div
                        ...
                        style={{ background: `linear-gradient(to top, transparent, hsl(var(--success) / 0.6))` }}
                      />
                    )}

                    {/* Pulse ring on comparing */}
                    {isComp && (
                      <motion.div
                  ...
                  {isPvt && (
                    <motion.span
                      ...
                      className="absolute -bottom-5 text-[7px] font-bold font-mono"
      ...
      <div className="glass rounded-xl p-4 flex flex-wrap gap-6">
        ...
        <div><p className="text-xs text-muted-foreground">Best</p><p className="font-mono text-success">{info.best}</p></div>
        ...
        <div><p className="text-xs text-muted-foreground">Stable</p><p className="font-mono text-info">{info.stable}</p></div>
        <div className="flex gap-4 ml-auto items-center flex-wrap">
          <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-primary" /><span className="text-[10px] text-muted-foreground">Unsorted</span></div>
          <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-accent" /><span className="text-[10px] text-muted-foreground">Comparing</span></div>
          ...
          <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full bg-success" /><span className="text-[10px] text-muted-foreground">Sorted</span></div>
        </div>
      </div>
